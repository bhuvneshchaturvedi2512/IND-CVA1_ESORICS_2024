...
uint64_t neg_threshold = 19;//4398046507008;
uint64_t pos_threshold = 19;//4398046507009;

int count_queries;
int no_of_attempts = 15;
int reactions = 0;
...
void example_bfv_basics()
{
    	...
    	for(int j = 1; j <= no_of_attempts; ++j)
	{
		...
		ans_incorrect = (ans_original.data()[i] + 1) % 1024;
    		...
    		x_encrypted.data(1)[0] = x_encrypted.data(1)[0] + 8796093013997;
	...
	for(int j = 1; j <= no_of_attempts; ++j)
	{
		...
		ans_incorrect = (ans_original.data()[i] - 1) % 1024;
    		...
    		x_encrypted.data(1)[0] = x_encrypted.data(1)[0] + 8796093013996;
