...
namespace seal
{
    ...
    void KeyGenerator::generate_sk(bool is_initialized)
    {
        ...
        if (!is_initialized)
        {
            ...
            Plaintext sk = secret_key_.data();
    
    	    FILE *Secret_Key = fopen("secret_key.txt","a");
    	    for(int i = 0; i < coeff_count; i++)
    	    {
    		if(sk[i] < 2)
    		    fprintf(Secret_Key, "%ld ", sk[i]);
    		else
    		    fprintf(Secret_Key, "%ld ", (sk[i] - 18014398492704769));
    	    }
    	    fprintf(Secret_Key, "\n");
    	    fclose(Secret_Key);
    	    ...
    	    Secret_Key = fopen("secret_key_ntt.txt","a");
    	    for(int i = 0; i < coeff_count; i++)
    	    {
    	    	fprintf(Secret_Key, "%lu ", sk[i]);
    	    }
    	    fprintf(Secret_Key, "\n");
    	    fclose(Secret_Key);*/
