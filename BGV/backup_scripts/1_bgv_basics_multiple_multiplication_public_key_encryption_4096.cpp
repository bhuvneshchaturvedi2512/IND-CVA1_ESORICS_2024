...
uint64_t neg_threshold = 34355200000;
uint64_t pos_threshold = 34355200000;
...
void example_bgv_basics()
{
    	...
    	for(int j = 1; j <= no_of_attempts; ++j)
	{
		...
		//ans_incorrect = (ans_original.data()[i] + 1) % 1024;
    		...
    		x_times_y_times_z_next.data(1)[0] = (x_times_y_times_z_next.data(1)[0] + 4608000) % q;
    		...
    		cout << "ans: 0x" << ans.to_string() << endl;
	...
	for(int j = 1; j <= no_of_attempts; ++j)
	{
		...
		decryptor.decrypt(x_times_y_times_z_next, ans_original);
		cout << "ans_original: 0x" << ans_original.to_string() << endl;
		//ans_incorrect = (ans_original.data()[i] - 1) % 1024;
