...
void example_bgv_basics()
{
    	...
    	for(int j = 1; j <= no_of_attempts; ++j)
	{
		...
		x = rand() % 4;
    		...
    		Plaintext zero_plain(uint64_to_hex_string(zero));
		...
		encryptor.encrypt(x_plain, x_encrypted);
    		...
    		evaluator.mod_switch_to_next_inplace(zero_encrypted);
    		
    		decryptor.decrypt(x_encrypted, ans_original);
    		
    		x_encrypted.data(1)[0] = (x_encrypted.data(1)[0] + 34359296000) % q;
    		x_encrypted.data()[i] = (x_encrypted.data()[i] + pos_threshold) % q;
    
    		evaluator.add_inplace(x_encrypted, zero_encrypted);
