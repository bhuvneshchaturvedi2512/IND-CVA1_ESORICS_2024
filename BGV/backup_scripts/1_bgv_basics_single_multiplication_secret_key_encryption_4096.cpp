...
void example_bgv_basics()
{
    	...
    	for(int j = 1; j <= no_of_attempts; ++j)
	{
		...
		//ans_incorrect = (ans_original.data()[i] - 1) % 1024;
    		...
    		x_times_y_next.data(1)[0] = (x_times_y_next.data(1)[0] + 34359296000) % q;
    		x_times_y_next.data()[i] = (x_times_y_next.data()[i] + pos_threshold) % q;
	...
	for(int j = 1; j <= no_of_attempts; ++j)
	{
		...
		//ans_incorrect = (ans_original.data()[i] + 1) % 1024;
    		...
    		x_times_y_next.data(1)[0] = (x_times_y_next.data(1)[0] + 34359296000) % q;
    		x_times_y_next.data()[i] = (x_times_y_next.data()[i] - neg_threshold) % q;
