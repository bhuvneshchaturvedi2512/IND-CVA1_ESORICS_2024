...
void example_bgv_basics()
{
    ...
    Ciphertext x_plus_y_encrypted_next;
    Ciphertext z_plus_w_encrypted_next;
    
    Plaintext ans;
    
    uint64_t ans_original;
    	...
    	for(int j = 1; j <= no_of_attempts; ++j)
	{
		...
		x = rand() % 4;
    		...
    		if(i == 0)
    		{
    			ans_original = x + y + z + w;
    		}
    		else
    		{
    			ans_original = 0;
    		}
    		//cout << "ans_original: " << ans_original << endl;
		...
		encryptor.encrypt(x_plain, x_encrypted);
    		...
    		evaluator.add_inplace(z_encrypted, w_encrypted);
    		
    		evaluator.mod_switch_to_next(x_encrypted, x_plus_y_encrypted_next);
    		evaluator.mod_switch_to_next(z_encrypted, z_plus_w_encrypted_next);
    		
    		x_plus_y_encrypted_next.data(1)[0] = (x_plus_y_encrypted_next.data(1)[0] + 34359296000) % q;
    		x_plus_y_encrypted_next.data()[i] = (x_plus_y_encrypted_next.data()[i] + pos_threshold) % q;
    	
    		evaluator.add_inplace(x_plus_y_encrypted_next, z_plus_w_encrypted_next);
    		
    		decryptor.decrypt(x_plus_y_encrypted_next, ans);
    		count_queries = count_queries + 1;
    		
    		//cout << "ans: " << ans.data()[i] << endl;
    		
    		//cout << "ans_original: 0x" << ans_original.to_string() << " ans: 0x" << ans.to_string() << endl;
    		if(ans.data()[i] != ans_original)//perturbation causes result to flip
    		{
	...
	for(int j = 1; j <= no_of_attempts; ++j)
	{
		...
		encryptor.encrypt(x_plain, x_encrypted);
    		...
    		x_plus_y_encrypted_next.data(1)[0] = (x_plus_y_encrypted_next.data(1)[0] + 34359296000) % q;
    		x_plus_y_encrypted_next.data()[i] = (x_plus_y_encrypted_next.data()[i] - neg_threshold) % q;
